...
final class Privacy_Lite_YouTube_Embeds {
    ...
    public function add_plugin_action_links(array $links): array {
        array_unshift($links, '<a href="' . esc_url(admin_url('options-general.php?page=privacy-lite-youtube-embeds')) . '">' . esc_html__('Settings', 'privacy-lite-youtube-embeds') . '</a>');
        return $links;
    }

    public function register_settings(): void {
        register_setting('plye_settings_group', self::OPTION_NAME, [
            'type' => 'array',
            'sanitize_callback' => [$this, 'sanitize_settings'],
            'default' => $this->default_settings(),
        ]);

        add_settings_section('plye_main_section', __('YouTube privacy replacement', 'privacy-lite-youtube-embeds'), function (): void {
    ...
    public function add_settings_page(): void {
        add_options_page(__('Privacy Lite YouTube Embeds', 'privacy-lite-youtube-embeds'), __('Privacy Lite YouTube', 'privacy-lite-youtube-embeds'), 'manage_options', 'privacy-lite-youtube-embeds', [$this, 'render_settings_page']);
    }
    ...
    public function render_settings_page(): void {
        ...
        settings_errors('plye_admin_notices');
        ?>
        <div class="wrap">
            ...
            <form method="post" action="options.php">
                <?php settings_fields('plye_settings_group'); do_settings_sections('plye_settings'); submit_button(); ?>
            </form>
            <hr>
            ...
            <form method="post" action="<?php echo esc_url(admin_url('admin-post.php')); ?>" style="margin-bottom: 1em;">
                ...
                <p class="description"><?php printf(esc_html__('Scans up to %d published public posts/pages per run and downloads missing local thumbnails.', 'privacy-lite-youtube-embeds'), self::MAX_SCAN_POSTS); ?></p>
            </form>
            <form method="post" action="<?php echo esc_url(admin_url('admin-post.php')); ?>">
                <input type="hidden" name="action" value="plye_clear_thumbnails">
                <?php wp_nonce_field('plye_clear_thumbnails'); ?>
                <?php submit_button(__('Clear local thumbnail cache', 'privacy-lite-youtube-embeds'), 'delete', 'submit', false, ['onclick' => "return confirm('" . esc_js(__('Delete all locally cached YouTube thumbnails?', 'privacy-lite-youtube-embeds')) . "');"]); ?>
    ...
    public function render_consent_toggle_field(): void {
        $settings = $this->settings();
        ?>
        <label><input type="checkbox" name="<?php echo esc_attr(self::OPTION_NAME); ?>[show_consent_text]" value="1" <?php checked($settings['show_consent_text']); ?>> <?php echo esc_html__('Show a short privacy message inside the placeholder.', 'privacy-lite-youtube-embeds'); ?></label>
        <?php
    }
    ...
    public function render_autoplay_field(): void {
        $settings = $this->settings();
        ?>
        <label><input type="checkbox" name="<?php echo esc_attr(self::OPTION_NAME); ?>[autoplay]" value="1" <?php checked($settings['autoplay']); ?>> <?php echo esc_html__('Start playback immediately after the user clicks the placeholder.', 'privacy-lite-youtube-embeds'); ?></label>
        <?php
    }
    ...
    public function handle_scan_thumbnails(): void {
        ...
        add_settings_error('plye_admin_notices', 'plye_scan_complete', sprintf(__('Scan complete. Posts scanned: %1$d. Videos found: %2$d. Existing thumbnails: %3$d. Generated: %4$d. Failed or unavailable: %5$d.', 'privacy-lite-youtube-embeds'), $result['posts_scanned'], $result['videos_found'], $result['existing'], $result['generated'], $result['failed']), 'success');
        set_transient('settings_errors', get_settings_errors(), 30);
        wp_safe_redirect(admin_url('options-general.php?page=privacy-lite-youtube-embeds&settings-updated=true'));
        exit;
    }
    ...
    public function handle_clear_thumbnails(): void {
        ...
        add_settings_error('plye_admin_notices', 'plye_clear_complete', sprintf(__('Local thumbnail cache cleared. Deleted files: %d.', 'privacy-lite-youtube-embeds'), $deleted), 'success');
        set_transient('settings_errors', get_settings_errors(), 30);
        wp_safe_redirect(admin_url('options-general.php?page=privacy-lite-youtube-embeds&settings-updated=true'));
        exit;
    }
    ...
    public function filter_render_block(string $block_content, array $block): string {
        ...
        return $video_id ? $this->render_placeholder($video_id, $this->extract_title_from_html($block_content)) : $block_content;
    }
    ...
    public function filter_oembed_html(string $html, string $url, array $attr, int $post_id): string {
        ...
        return $video_id ? $this->render_placeholder($video_id, $this->extract_title_from_html($html)) : $html;
    }
    ...
    public function filter_content_iframes(string $content): string {
        ...
        return preg_replace_callback('#<iframe\b[^>]*\bsrc=["\']([^"\']*(?:youtube\.com|youtube-nocookie\.com|youtu\.be)[^"\']*)["\'][^>]*>\s*</iframe>#i', function (array $matches): string {
            $video_id = $this->extract_video_id($matches[1]);
            return $video_id ? $this->render_placeholder($video_id, $this->extract_title_from_html($matches[0])) : $matches[0];
        }, $content) ?: $content;
    }
    ...
    private function scan_content_for_thumbnails(int $limit): array {
        $result = ['posts_scanned' => 0, 'videos_found' => 0, 'existing' => 0, 'generated' => 0, 'failed' => 0];
        $query = new WP_Query([
        ...
        foreach ($query->posts as $post_id) {
            ...
            foreach ($video_ids as $video_id) {
                if ($this->thumbnail_exists($video_id)) {
                    $result['existing']++;
                } elseif ($this->get_local_thumbnail($video_id)) {
                    $result['generated']++;
                } else {
                    $result['failed']++;
                }
    ...
    private function download_thumbnail(string $video_id, string $destination): bool {
        foreach (['maxresdefault.jpg', 'sddefault.jpg', 'hqdefault.jpg'] as $candidate) {
            ...
            $response = wp_safe_remote_get($url, ['timeout' => 8, 'redirection' => 3, 'user-agent' => 'Privacy Lite YouTube Embeds/' . self::VERSION . '; ' . home_url('/')]);
    ...
    private function extract_video_ids(string $value): array {
        ...
        $video_ids = [];
        foreach (['#youtu\.be/([A-Za-z0-9_-]{6,20})#i', '#youtube(?:-nocookie)?\.com/embed/([A-Za-z0-9_-]{6,20})#i', '#youtube(?:-nocookie)?\.com/shorts/([A-Za-z0-9_-]{6,20})#i', '#[?&]v=([A-Za-z0-9_-]{6,20})#i'] as $pattern) {
